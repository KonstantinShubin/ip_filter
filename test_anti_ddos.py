...
def test_banned_time_over_max(reset_table):
    ...
    time.sleep(anti.first_time_ban - 25)
    check("1")
    assert round(get_banned_time("1")) == anti.max_ban_time
    assert_current_time("1")
    remaining_time = time.strftime("%H hours, %M minutes and %S seconds", time.gmtime(get_banned_time("1")))
    assert check("1") == (anti.SOFT_BAN, "Your ban time was increased by {} and now is {}. Please try again later. ".format(anti.ban_time_coef, remaining_time))
    assert get_status("1") == 1
...
def test_not_banned_ip(reset_table):
    ...
    assert check("1") == (anti.OK, "Welcome. Haven't seen you for a while. ")
    assert get_num_logins("1") == 1
    assert_current_time("1")
...
def test_num_logins_over_limit(reset_table):
    ...
    assert get_num_logins("1") == 1
    check("1")
    assert get_num_logins("1") == 2
    remaining_time = time.strftime("%H hours, %M minutes and %S seconds", time.gmtime(anti.first_time_ban))
    assert check("1") == (anti.SOFT_BAN, f"You have been blocked for {remaining_time}. " \
    ...
    assert get_num_logins("1") == anti.num_softban + 1
    assert get_status("1") == 1
    assert_current_time("1")
    assert get_banned_time("1") == anti.first_time_ban
...
def test_good_ip_returned_again(reset_table):
    check("1")
    assert check("1") == (anti.OK, "Hi again. ")
    assert get_num_logins("1") == 2
